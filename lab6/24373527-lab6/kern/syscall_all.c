...
int sys_mem_map(u_int srcid, u_int srcva, u_int dstid, u_int dstva, u_int perm)
{
	...
	int r1 = envid2env(srcid, &srcenv, 1);
	if (r1 != 0)
	{
		return r1;
	}
	...
	int r2 = envid2env(dstid, &dstenv, 1);
	if (r2 != 0)
	{
		return r2;
	}
...
int sys_exofork(void)
{
	...
	int r1 = env_alloc(&e, curenv->env_id);
	if (r1 != 0)
	{
		return r1;
	}
...
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm)
{
	...
	if (srcva != 0)
	{
		...
		int r2 = page_insert(e->env_pgdir, e->env_asid, p, e->env_ipc_dstva, perm);
		if (r2 != 0)
		{
			return r2;
		}
...
int sys_write_dev(u_int va, u_int pa, u_int len)
{
	...
	if (!(pa >= 0x180001F0 && pa < 0x180001F8) && !(pa == 0x180003F8) && !(pa == 0x180003FD))
	{
		return -E_INVAL;
	}
	if (((pa >= 0x180001F1 && pa < 0x180001F8) || (pa == 0x180003FD)) && len != 1)
	{
		return -E_INVAL;
	}
	if (len == 4)
